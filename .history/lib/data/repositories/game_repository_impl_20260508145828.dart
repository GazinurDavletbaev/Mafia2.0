...
Future<GameState> getCurrentGameState() async {
  ...
  AppLogger.d('REPOSITORY getCurrentGameState: currentSpeaker=${state?.currentSpeakerSeat}');
  return state ?? GameState.initial();
}

@override
Future<void> saveCurrentGameState(GameState state) async {
  AppLogger.d('REPOSITORY saveCurrentGameState: currentSpeaker=${state.currentSpeakerSeat}');
