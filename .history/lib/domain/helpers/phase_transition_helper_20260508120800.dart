...
class PhaseTransitionHelper {
  ...
  GameState _nextDay(GameState state) {
    ...
    // Если мы в speeches и это последний индекс — переходим на голосование
    ...
    // Иначе — инициализируем речи (только при первом входе в день)
    return _initializeSpeeches(state);
  }

  GameState _initializeSpeeches(GameState state) {
    // Определяем стартовое место для речей
    int startSeat;
    ...
    // Ищем первого живого начиная с startSeat
    int? firstAliveSeat;
    for (int i = 0; i < 10; i++) {
