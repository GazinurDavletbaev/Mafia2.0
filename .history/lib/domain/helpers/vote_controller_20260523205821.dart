...
class VoteController {
  ...
  static VoteResult determineResult(Map<int, int> votes, int aliveCount) {
    if (votes.isEmpty) {
      return VoteResult.noCandidates(); // ← используй фабрику
    }
    ...
    // Если один лидер
    if (winners.length == 1) {
      return VoteResult.winner(winners.first, maxVotes); // ← используй фабрику
    }

    // Если несколько лидеров → перестрелка
    return VoteResult.tieBreak(winners, maxVotes); // ← используй фабрику
  }
}
