...
class VoteController {
  ...
  static VoteResult determineResult(Map<int, int> votes, int aliveCount) {
  if (votes.isEmpty) {
    return VoteResult(winnerSeat: null, seats: [], type: VoteResultType.noCandidates);
  }
  
  final maxVotes = votes.values.reduce((a, b) => a > b ? a : b);
  final winners = votes.entries.where((e) => e.value == maxVotes).map((e) => e.key).toList();
  
  // Если один лидер
  if (winners.length == 1) {
    return VoteResult(
      winnerSeat: winners.first,
      seats: winners,
      type: VoteResultType.winner,
    );
  }
  
  // Если несколько лидеров → перестрелка
  return VoteResult(
    winnerSeat: null,
    seats: winners,
    type: VoteResultType.tieBreak,  // всегда tieBreak, не eliminationVote
  );
ЪЪ
}
