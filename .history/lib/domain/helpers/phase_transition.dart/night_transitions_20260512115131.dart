...
class NightTransitions {
  static GameState next(GameState state) {
    ...
    AppLogger.d(
      'NightTransitions.next() → isFirstNight=$isFirstNight, day=${state.currentDay}, subPhase=${state.currentSubPhase}',
    );
  ...
  static GameState previous(GameState state) {
    final isFirstNight = state.currentDay == 0;
    AppLogger.d(
      'NightTransitions.previous() → isFirstNight=$isFirstNight, day=${state.currentDay}, subPhase=${state.currentSubPhase}',
    );
    return isFirstNight
        ? _previousFirstNight(state)
        : _previousRegularNight(state);
  }

  static GameState _nextFirstNight(GameState state) {
    AppLogger.d(
      '_nextFirstNight() → currentSubPhaseIndex=${state.currentSubPhaseIndex}',
    );
    const subPhases = [
  ...
  static GameState _nextRegularNight(GameState state) {
    AppLogger.d(
      '_nextRegularNight() → currentSubPhaseIndex=${state.currentSubPhaseIndex}, day=${state.currentDay}',
    );
    const subPhases = [
      SubPhase.mafiaShoot,
      SubPhase.donCheck,
    ...
    AppLogger.d(
      '  ночь закончена → переход в день, hasKill=$hasKill, nextSubPhase=$nextSubPhase',
    );
  ...
  static GameState _previousFirstNight(GameState state) {
    AppLogger.d(
      '_previousFirstNight() → currentSubPhaseIndex=${state.currentSubPhaseIndex}',
    );
    const subPhases = [
      SubPhase.roleDistribution,
      SubPhase.contract,
  ...
  static GameState _previousRegularNight(GameState state) {
    AppLogger.d(
      '_previousRegularNight() → currentSubPhaseIndex=${state.currentSubPhaseIndex}',
    );
    const subPhases = [
      SubPhase.mafiaShoot,
      SubPhase.donCheck,
    ...
    AppLogger.d(
      '  возврат из ночи в день, wasBestMove=$wasBestMove, day=${state.currentDay}',
    );
