...
class PhaseTransitionHelper {
 ...
 GameState _nextDay(GameState state) {
  ...
  if (state.currentSubPhaseIndex < subPhases.length - 1) {
    ...
    // Если переходим на speeches — инициализируем
    if (nextSubPhase == SubPhase.speeches && state.currentSubPhase != SubPhase.speeches) {
 ...
 GameState _initializeSpeeches(GameState state) {
  AppLogger.d('_initializeSpeeches called, currentDay=${state.currentDay}, dayStarterSeat=${state.dayStarterSeat}');
  ...
  AppLogger.d('_initializeSpeeches: startSeat=$startSeat, firstAlive=$firstAliveSeat');
