...
print('=== _dayOrder список: $_dayOrder');
print('=== содержит eliminationVote? ${_dayOrder.contains(SubPhase.eliminationVote)}');
    ...
    if (phaseHistory.isEmpty) {
      ...
      if (_night0Order.contains(currentPhase)) {
        ...
        print('=== _dayOrder: currentPhase = $currentPhase');

        // СПЕЦИАЛЬНАЯ ОБРАБОТКА ДЛЯ eliminationVote
        if (currentPhase == SubPhase.eliminationVote) {
          print('=== eliminationVote СРАБОТАЛ!');
          final totalAlive = currentState.players
              .where((p) => p.isAlive)
              .length;
          final majority = (totalAlive ~/ 2) + 1;
          print(
            '=== totalAlive=$totalAlive, majority=$majority, eliminationVotes=${currentState.eliminationVotes}',
          );
          ...
          print('=== last phase: ${_dayOrder.last}');
          ...
          print('=== обычный переход по _dayOrder');
