      ...
      if (_night0Order.contains(currentPhase)) {
        ...
        if (currentPhase == _dayOrder.last) {
          ...
          if (next != null) {
            ...
            print('phaseHistory.last = ${currentState.phaseHistory.last}');
            nextPhase = next;
            newPhaseHistory = List.from(phaseHistory)..add(nextPhase);
          } else {
            nextPhase = SubPhase.mafiaShoot;
...
if (currentState.currentSubPhase == SubPhase.eliminationVote) {
  ...
  if (currentState.eliminationVotes >= majority) {
    ...
    // Голосов НЕ хватило → ночь
    nextPhase = SubPhase.mafiaShoot;
    // ВАЖНО: добавляем mafiaShoot в историю!
    newPhaseHistory = List.from(phaseHistory)..add(nextPhase);
  }
}
    // Обычный переход
