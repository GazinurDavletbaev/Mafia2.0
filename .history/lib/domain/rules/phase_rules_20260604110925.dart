...
class PhaseRules {
  ...
  Future<GameState> calculateNextState(GameState currentState) async {
      ...
      if (_night0Order.contains(currentPhase)) {
        ...
        // ПОСЛЕДНЯЯ ФАЗА ДНЯ
          ...
          if (candidates.isEmpty) {
            ...
            AppLogger.d('Day0, 1 candidate -> contract');
          } else if (candidates.length == 1 && !isDay0) {
            nextPhase = SubPhase.finalWord;
    ...
    if (nextPhase == SubPhase.bestMove) {
      ...
      final killedSeat = nightActions.length >= 3 ? nightActions[nightActions.length - 3] : null;
      return currentState.copyWith(
        phaseHistory: newPhaseHistory,
        currentSubPhase: nextPhase,
    ...
    if (nextPhase == SubPhase.finalWord) {
      final winner = currentState.votes.entries.reduce((a, b) => a.value > b.value ? a : b).key;
      return currentState.copyWith(
        phaseHistory: newPhaseHistory,
        currentSubPhase: nextPhase,
        currentDay: newPhaseHistory.where((p) => p == SubPhase.mafiaShoot).length,
    ...
    return currentState.copyWith(
      ...
      currentSpeakerSeat: currentState.speechHistory.isNotEmpty ? currentState.speechHistory.last : null,
