...
class PhaseRules {
  ...
  print('=== NIGHT ACTIONS BEFORE ===');
  print('nightActions: ${currentState.nightActions}');
  print('currentDay: ${currentState.currentDay}');
  print('currentPhase: $currentPhase');
  
  // ✅ ОПРЕДЕЛЯЕМ ИНДЕКС ДЛЯ ТЕКУЩЕЙ ФАЗЫ
  int index = 0;
  switch (currentPhase) {
    case SubPhase.mafiaShoot:
      index = currentState.currentDay * 3 + 0;
      break;
    case SubPhase.donCheck:
      index = currentState.currentDay * 3 + 1;
      break;
    case SubPhase.sheriffCheck:
      index = currentState.currentDay * 3 + 2;
      break;
    default:
      break;
  }
  
  // ✅ ПРОВЕРЯЕМ, ЕСТЬ ЛИ ЗНАЧЕНИЕ ДЛЯ ЭТОЙ ФАЗЫ
  final nightActions = List<int>.from(currentState.nightActions ?? []);
  while (nightActions.length <= index) {
    nightActions.add(0);  // добавляем 0, если не хватает
  }
  
  // Если значение по индексу равно -1 или 0 (не введено) — ставим 0
  if (nightActions[index] == -1 || nightActions[index] == 0) {
    nightActions[index] = 0;
  }
  ...
  if (next == SubPhase.donCheck) {
    ...
    if (!don.isAlive) {
      print('hi don');
      // Дон мертв — добавляем 0 для donCheck
      final newNightActions = List<int>.from(nightActions);
      while (newNightActions.length <= currentState.currentDay * 3 + 1) {
        newNightActions.add(0);
      }
      newNightActions[currentState.currentDay * 3 + 1] = 0;
      
      print('=== NIGHT ACTIONS AFTER (don dead) ===');
      print('newNightActions: $newNightActions');
