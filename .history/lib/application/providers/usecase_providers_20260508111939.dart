...
import '../../domain/usecases/elimination_usecase.dart';
import '../../domain/usecases/revote_usecase.dart';
import '../../domain/usecases/tie_break_usecase.dart';
import '../../domain/usecases/voting_usecase.dart';
...
final votingUsecaseProvider = Provider<VotingUsecase>((ref) {
  final repository = ref.read(gameRepositoryProvider);
  return VotingUsecase(repository: repository);
});

final revoteUsecaseProvider = Provider<RevoteUsecase>((ref) {
  final repository = ref.read(gameRepositoryProvider);
  return RevoteUsecase(repository: repository);
});

final tieBreakUsecaseProvider = Provider<TieBreakUsecase>((ref) {
  final repository = ref.read(gameRepositoryProvider);
  return TieBreakUsecase(repository: repository);
});

final eliminationVoteUsecaseProvider = Provider<EliminationVoteUsecase>((ref) {
  final repository = ref.read(gameRepositoryProvider);
  return EliminationVoteUsecase(repository: repository);
});
