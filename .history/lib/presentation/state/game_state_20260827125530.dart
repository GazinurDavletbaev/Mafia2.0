...
class GameState {
  ...
  final bool showAllRoles; // ← ДОБАВИТЬ
  ...
  // 🔥 ИГРОК, КОТОРЫЙ УСТРОИЛ ППК (null — если ППК не было)
  final int? ppkPlayerSeat;
  ...
  GameState copyWith({
    ...
    String? stageName,
    int? ppkPlayerSeat, // 🔥 ДОБАВЛЯЕМ
    bool? showAllRoles, // ← ДОБАВИТЬ
  }) {
    return GameState(
      game: game ?? this.game,
      ...
      ppkPlayerSeat: ppkPlayerSeat ?? this.ppkPlayerSeat, // 🔥 ДОБАВЛЯЕМ
      showAllRoles: showAllRoles ?? this.showAllRoles, // ← ДОБАВИТЬ
    );
  }
