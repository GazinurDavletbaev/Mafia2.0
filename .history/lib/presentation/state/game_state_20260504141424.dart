...
import '../../data/local/models/sub_phases.dart';
...
class GameState {
  ...
  factory GameState.initial() {
    // Временно создаём 10 игроков с тестовыми именами
    const names = [
    ...
    return GameState(
      ...
      currentSubPhase: SubPhase.roleDistribution,  // ← здесь
  ...
  GameState addPendingBestMove(List<int> threeSeats) {
    final bestMove = BestMove(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      gameId: game?.id.toString() ?? '',
      killedSeatNumber: threeSeats[0],   // ← было killedSeat, стало killedSeatNumber
      suspectSeat1: threeSeats[0],       // ← было suspect1, стало suspectSeat1
      suspectSeat2: threeSeats[1],       // ← было suspect2, стало suspectSeat2
      suspectSeat3: threeSeats[2],       // ← было suspect3, стало suspectSeat3
    );
