...
class ProtocolPlayersTable extends StatefulWidget {
  final GameState gameState;
  final List<double> bonusPoints;
  ...
  @override
  State<ProtocolPlayersTable> createState() => _ProtocolPlayersTableState();
}

class _ProtocolPlayersTableState extends State<ProtocolPlayersTable> {
  late List<double> _bonusPoints;

  @override
  void initState() {
    super.initState();
    _bonusPoints = List<double>.from(widget.bonusPoints);
  }
  ...
  TableRow _buildPlayerRow(BuildContext context, PlayerModel player) {
    ...
    final points = ProtocolHelper.calculatePoints(player, widget.gameState.winner);
    final isRemoved = ProtocolHelper.isRemoved(player, widget.gameState);
    final hasPpk = ProtocolHelper.hasPpk(player, widget.gameState);
    final currentBonus = _bonusPoints.length > index ? _bonusPoints[index] : 0.0;
    ...
    return TableRow(
      children: [
        ...
        _buildBonusCell(context, index, player, isRemoved, hasPpk, currentBonus),
      ],
    );
  }
  ...
  Widget _buildBonusCell(
    ...
    return _buildBonusDropdown(context, index, currentBonus);
  }
        ...
        child: DropdownButton<String>(
          ...
          onChanged: (newValue) {
            if (newValue != null && newValue.isNotEmpty) {
              setState(() {
                widget.onRemovedRuleChanged(player.seatNumber, newValue);
                widget.onNoteAdded(player, newValue);
              });
            }
          },
        ),
  ...
  Widget _buildBonusDropdown(BuildContext context, int index, double currentBonus) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
          ...
          onChanged: (newValue) {
            if (newValue != null) {
              setState(() {
                ...
                widget.onBonusChanged(index, newValue);
                widget.onBonusNoteAdded(index, newValue);
              });
            }
          },
        ),
