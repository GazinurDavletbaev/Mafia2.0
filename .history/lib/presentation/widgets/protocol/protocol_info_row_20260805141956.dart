...
class ProtocolInfoRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            _infoRow(context, 'ПОБЕДИВШАЯ КОМАНДА    ', winner, color: winnerColor),
            Divider(
  ...
  Widget _buildNightActionsTable(BuildContext context, List<Map<String, dynamic>> nights, bool isDark) {
    final theme = Theme.of(context);
    final primaryColor = theme.primaryColor;
        ...
        children: [
          TableRow(
            ...
            children: [
              ...
              ...nights.map((n) => _tableCell(context, '${n['night']}', isHeader: true)).toList(),
            ],
          ),
          TableRow(
            children: [
              _tableCell(context, 'Стрельба', isHeader: true),
              ...nights.map((n) => _tableCell(context, n['kill'] as String)).toList(),
            ],
          ),
          TableRow(
            children: [
              _tableCell(context, 'Дон', isHeader: true),
              ...nights.map((n) => _tableCell(context, n['don'] as String)).toList(),
            ],
          ),
          TableRow(
            children: [
              _tableCell(context, 'Шериф', isHeader: true),
              ...nights.map((n) => _tableCell(context, n['sheriff'] as String)).toList(),
            ],
          ),
        ],
      ),
  ...
  Widget _tableCell(BuildContext context, String text, {bool isHeader = false}) {
    ...
    return Padding(
      ...
      child: Text(
        text,
        style: TextStyle(
          color: isHeader ? primaryColor : (isDark ? Colors.white : Colors.black87),
