...
class _ProtocolPlayersTableState extends State<ProtocolPlayersTable> {
  ...
  TableRow _buildPlayerRow(BuildContext context, PlayerModel player) {
    ...
    final points =
        ProtocolHelper.calculatePoints(player, widget.gameState.winner);
    ...
    final currentBonus =
        _bonusPoints.length > index ? _bonusPoints[index] : 0.0;
    ...
    return TableRow(
      children: [
        ...
        _buildBonusCell(
            context, index, player, isRemoved, hasPpk, currentBonus),
      ],
    );
  }
  ...
  Widget _buildBonusDropdown(
      BuildContext context, int index, double currentBonus) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
