...
class ProtocolVotingTable extends StatelessWidget {
  ...
  Widget _buildVoteCard(
    ...
    // 🔥 ПЕРВЫЙ РАУНД
    // 🔥 ПЕРВЫЙ РАУНД
final Map<int, int> firstRound = rounds.isNotEmpty ? Map<int, int>.from(rounds[0]) : {};
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            if (rounds.length > 1) ...[
              // 🔥 ПЕРВОЕ ПЕРЕГОЛОСОВАНИЕ: Пере- + голос.
              // 🔥 ПЕРВОЕ ПЕРЕГОЛОСОВАНИЕ
final Map<int, int> secondRound = rounds.length > 1 ? Map<int, int>.from(rounds[1]) : {};
final List<int> secondKeys = secondRound.keys.toList()..sort();
              ...
              if (rounds.length > 2) ...[
                for (int i = 2; i < rounds.length; i++) ...[
                  ...
                  final keys = round.keys.toList()..sort();
