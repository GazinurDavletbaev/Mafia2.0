...
class ProtocolPlayersTable extends StatelessWidget {
  ...
  Widget _buildPointsCell(BuildContext context, int index) {
    ...
    final points =
        gameState.players[index].team == (isRedWon ? 'red' : 'black') ? 1 : 0;
  ...
  Widget _buildBonusPointsCell(
      BuildContext context, int index, PlayerModel player) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
    final isRemoved = gameState.removedPlayers
        .any((rp) => rp.seatNumber == player.seatNumber);
  ...
  Widget _buildRemovedDropdown(
      BuildContext context, int index, PlayerModel player, bool isDark) {
