...
class ProtocolVotingTable extends StatelessWidget {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // 🔥 ПЕРВЫЙ РАУНД
            _buildRoundRow('Игрок', firstRound, isDark, true),
            _buildRoundRow('Голоса', firstRound, isDark, true),
            ...
            if (rounds.length > 1) ...[
              ...
              for (int i = 1; i < rounds.length; i++) ...[
                ...
                _buildRoundRow('Пере-', round, isDark, true),
                _buildRoundRow('голос.', round, isDark, true),
              ],
            ],
  ...
  Widget _buildRoundRow(
    ...
    // 🔥 БЕРЁМ ВСЕ КЛЮЧИ ИЗ round (ЭТО НОМЕРА ИГРОКОВ)
    final List<int> seats = round.keys.toList()..sort();

    // 🔥 ЕСЛИ ПУСТО — ПОКАЗЫВАЕМ ПРОСТО ПРОЧЕРКИ
    if (seats.isEmpty) {
      return Padding(
