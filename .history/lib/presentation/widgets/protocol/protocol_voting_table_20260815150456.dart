...
class ProtocolVotingTable extends StatelessWidget {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // 🔥 ПЕРВЫЙ РАУНД
            _buildRoundRow('Игрок', firstRoundKeys, firstRound, isDark, true),
            _buildRoundRow('Голоса', firstRoundKeys, firstRound, isDark, true),
            const SizedBox(height: 4),

            // 🔥 ЕСЛИ ЕСТЬ ПЕРЕГОЛОСОВАНИЯ
            if (rounds.length > 1) ...[
              _buildRevoteRows(rounds, isDark),
            ],
  ...
  // 🔥 СТРОКИ ПЕРЕГОЛОСОВАНИЙ
  Widget _buildRevoteRows(List<Map<int, int>> rounds, bool isDark) {
    ...
    for (int i = 1; i < rounds.length; i++) {
      ...
      // 🔥 ПЕРВЫЙ РАУНД ПЕРЕГОЛОСОВАНИЯ — ПОКАЗЫВАЕМ "Пере-" и "голос."
      if (i == 1) {
        rows.add(_buildRoundRow('Пере-', keys, round, isDark, true));
        rows.add(_buildRoundRow('голос.', keys, round, isDark, true));
        rows.add(const SizedBox(height: 4));
      } else {
        // 🔥 СЛЕДУЮЩИЕ — ТОЛЬКО ЦИФРЫ
        rows.add(_buildRoundRow('', keys, round, isDark, true));
        rows.add(_buildRoundRow('', keys, round, isDark, true));
        rows.add(const SizedBox(height: 4));
      }
    }
  ...
  Widget _buildRoundRow(
    ...
    // 🔥 ПОЛУЧАЕМ ВСЕ ВОЗМОЖНЫЕ НОМЕРА ОТ 1 ДО 10
    final allSeats = List.generate(10, (i) => i + 1);

    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 1),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.start, // 🔥 ВЛЕВО
        children: [
          // 🔥 ЛЕЙБЛ
          if (label.isNotEmpty)
            SizedBox(
          ...
          ...allSeats.map((seat) {
            ...
            String displayValue;
            if (!hasVote) {
              displayValue = '-'; // 🔥 ЕСЛИ ИГРОК ВЫБЫЛ — СТАВИМ "-"
            } else if (value == 0) {
              displayValue = '0';
            } else {
