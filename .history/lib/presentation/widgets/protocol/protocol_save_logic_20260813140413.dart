...
class ProtocolSaveLogic {
  ...
  void addBonusNote(int index, double value) {
    ...
    // 🔥 ЕСЛИ ЗАПИСИ НЕТ — ИЩЕМ ПУСТОЕ МЕСТО
    for (int i = 0; i < noteControllers.length; i++) {
    ...
    // 🔥 ЕСЛИ ВСЕ ЗАНЯТЫ — ДОБАВЛЯЕМ В ПЕРВУЮ
...
// 🔥 СОХРАНИТЬ ЛОКАЛЬНО (БЕЗ СЕРВЕРА)
Future<void> saveLocalProtocol(BuildContext context) async {
  ...
  try {
    // 🔥 СОХРАНЯЕМ ЛОКАЛЬНО В HIVE
    final localSource = ref.read(protocolLocalSourceProvider);
    await localSource.saveProtocol(GameProtocol.fromJson(data));
