...
if (rounds.length > 1) {
  ...
  children.add(_buildRoundRow('Пере-', secondKeys, secondRound, isDark, showValue: true));
  children.add(_buildRoundRow('голос.', secondKeys, secondRound, isDark, showValue: true));
  ...
  if (rounds.length > 2) {
    for (int i = 2; i < rounds.length; i++) {
      ...
      children.add(_buildRoundRow('', keys, round, isDark, showValue: true));
      children.add(_buildRoundRow('', keys, round, isDark, showValue: true));
      children.add(const SizedBox(height: 4));
    }
  }

  // Добавляем все в Column
