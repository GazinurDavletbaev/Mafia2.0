...
class ProtocolVotingTable extends StatelessWidget {
        ...
        child: Column(
          ...
          children: [
            ...
            ...days.map((day) {
              ...
              return _buildVoteDayCard(context, day, dayData, voteNumber, isDark);
            }),
          ],
        ),
      ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            _buildRoundRow('Игрок', const <int, int>{}, isDark),
            _buildRoundRow('Голоса', const <int, int>{}, isDark),
  ...
  Widget _buildVoteCard(
    ...
    final firstRound = rounds.isNotEmpty ? Map<int, int>.from(rounds[0]) : <int, int>{};
    final allSeats = _getAllSeats(rounds);
    ...
    children.add(_buildFullRoundRow('Игрок', firstRound, allSeats, isDark));
    children.add(_buildFullRoundRow('Голоса', firstRound, allSeats, isDark));

    if (rounds.length > 1) {
      children.add(const SizedBox(height: 4));
      for (int i = 1; i < rounds.length; i++) {
        final round = Map<int, int>.from(rounds[i]);
        children.add(_buildFullRoundRow('Пере-', round, allSeats, isDark));
        children.add(_buildFullRoundRow('голос.', round, allSeats, isDark));
      }
    }
  ...
  List<int> _getAllSeats(List<Map<int, int>> rounds) {
    final Set<int> allSeats = {};
    for (final round in rounds) {
      allSeats.addAll(round.keys);
    }
    final List<int> sorted = allSeats.toList()..sort();
    return sorted;
  }

  Widget _buildFullRoundRow(
    String label,
    Map<int, int> round,
    List<int> allSeats,
    bool isDark,
  ...
  Widget _buildRoundRow(
    ...
    final List<int> seats = round.keys.toList()..sort();
