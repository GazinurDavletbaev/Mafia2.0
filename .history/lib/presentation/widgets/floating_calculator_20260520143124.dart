// lib/presentation/widgets/floating_calculator.dart
...
class _FloatingCalculatorState extends ConsumerState<FloatingCalculator> {
  ...
  void _onNumberTap(int value) {
    ...
    final vm = ref.read(gameViewModelFamily(widget.gameId).notifier);
    
    if (gameState.isVotingActive) {
      vm.submitVote(value);
    } else if (gameState.currentSubPhase == SubPhase.mafiaShoot ||
  ...
  void _minimizeWindow() {
    // Логика сворачивания окна (Desktop)
    // Вызвать метод из _vm или использовать window_manager
  }
}