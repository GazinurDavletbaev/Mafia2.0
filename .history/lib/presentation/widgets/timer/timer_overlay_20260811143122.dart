...
class _TimerOverlayState extends ConsumerState<TimerOverlay> {
  ...
  void _syncTimer() {
    ...
    // 🔥 ПРОВЕРЯЕМ: ЕСЛИ ТАЙМЕР УЖЕ ЗАПУЩЕН И ОСТАЛОСЬ ВРЕМЯ
    if (timerState.isRunning && timerState.remainingSeconds > 0) {
      // ✅ ПРОДОЛЖАЕМ С ТЕКУЩИМ ЗНАЧЕНИЕМ
      _currentSeconds = timerState.remainingSeconds;
      _timerStarted = true;

      // ✅ ПРИ НЕОБХОДИМОСТИ ОБНОВЛЯЕМ КОЛБЭК
    ...
    // 🔥 ОБНОВЛЯЕМ floatingTimerProvider ДЛЯ TimerOverAll
    ref.read(floatingTimerProvider.notifier).state = _currentSeconds;
  }
  ...
  Widget build(BuildContext context) {
    ...
    // 🔥 СЛЕДИМ ЗА ИЗМЕНЕНИЯМИ ТАЙМЕРА
    if (timerState.isRunning && _currentSeconds != currentSeconds) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          setState(() {
    ...
    // 🔥 ЕСЛИ ТАЙМЕР ОСТАНОВЛЕН — ПОКАЗЫВАЕМ 0
    if (!timerState.isRunning && currentSeconds <= 0) {
