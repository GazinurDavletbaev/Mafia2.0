...
class PhaseHeader extends StatelessWidget {
  ...
  // Если это речи или перестрелка, добавляем номер игрока
  if (subPhase == SubPhase.speeches || subPhase == SubPhase.tieBreak) {
    final speakerText = currentSpeaker != null ? ' | Игрок $currentSpeaker' : '';
    
    switch (phase) {
      case Phase.night:
        return 'Ночь $currentDay | $subPhaseTitle$speakerText';
      case Phase.day:
        return 'День $currentDay | $subPhaseTitle$speakerText';
    }
  }
  
  // Обычный заголовок без номера игрока
  switch (phase) {
    case Phase.night:
