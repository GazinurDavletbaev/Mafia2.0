  ...
  void _onNumberTap(int value) {
    ...
    if (_state.isVotingActive) {
      ...
      if (controller != null) {
        ...
        // Если голоса закончились (0 осталось), оставшимся кандидатам ставим 0
        if (controller.totalVotes == aliveCount) {
          // Переходим к следующему кандидату и ставим 0, пока есть кандидаты
        ...
        // Если это последний кандидат
        if (controller.currentIndex == controller.totalCandidates - 1) {
          final newRemaining = aliveCount - controller.totalVotes;
          if (newRemaining > 0) {
