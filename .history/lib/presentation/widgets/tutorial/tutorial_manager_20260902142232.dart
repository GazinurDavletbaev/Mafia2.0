...
class TutorialManager {
  ...
  static Future<void> startTutorials({
    ...
    // 🔥 НЕ ФИЛЬТРУЕМ! ПОКАЗЫВАЕМ ВСЕ ПОДСКАЗКИ
  ...
  static void _showNext() {
    print('🔍 _showNext called, index: $_currentIndex/${_currentQueue.length}');

    if (_currentIndex >= _currentQueue.length) {
      print('✅ Все подсказки показаны!');
      _isShowing = false;
    ...
    _overlayEntry = OverlayEntry(
      builder: (context) => TutorialOverlay(
        ...
        onClose: () async {
          ...
          // 🔥 ПРИНУДИТЕЛЬНО ПОКАЗЫВАЕМ СЛЕДУЮЩУЮ
