...
class _GameScreenState extends ConsumerState<GameScreen> {
  ...
  void initState() {
    ...
    // Инициализируем игру с данными из initialData
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final players = widget.initialData.gameState.players;
      final hasNames = players.any((p) => p.name.isNotEmpty);
      if (!hasNames && widget.initialData.playerNames.isNotEmpty) {
        _vm.initializeGame(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              _dialogShown = false;
              _notifyGameStateChanged();
              // Переключаемся на вкладку "Протокол" в LobbyScreen
              final lobbyState = context.findAncestorStateOfType<_LobbyScreenState>();
              if (lobbyState != null) {
                // Обновляем состояние и переключаемся на протокол
                lobbyState._updateGameData(
                  GameData(
                    tournamentName: widget.initialData.tournamentName,
                    stageName: widget.initialData.stageName,
                    tableNumber: widget.initialData.tableNumber,
                ...
                // Переключаемся на вкладку "Протокол" (индекс 4)
                lobbyState.setSelectedIndex(4);
              }
            },
            child: const Text(
              '📋 ПРОТОКОЛ ИГРЫ',
