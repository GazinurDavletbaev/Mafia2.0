...
class _GameProtocolScreenState extends State<GameProtocolScreen> {
  ...
  if (widget.gameState.voteHistory.isEmpty) {
    ...
    widget.gameState.voteHistory.forEach((day, voteDay) {
      print('  День $day:');
      print('    vote: ${voteDay.vote}');
      print('    revote: ${voteDay.revote}');
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Голосование $voteNumber (день $day)',
              style: const TextStyle(
                color: Colors.white,
                fontSize: 14,
                fontWeight: FontWeight.bold,
            ...
            _buildVoteRow('Голосование', dayData.vote),

            // Переголосование (если есть)
            if (dayData.revote != null && dayData.revote!.isNotEmpty) ...[
              const SizedBox(height: 4),
              _buildVoteRow('Переголосование', dayData.revote!),
            ],
  ...
  void _saveProtocol() async {
    final data = {
      ...
      'voteHistory': widget.gameState.voteHistory.map((day, dayData) {
        return MapEntry(day.toString(), dayData.toJson());
      }),
    };
