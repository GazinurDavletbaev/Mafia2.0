...
class _GameScreenState extends ConsumerState<GameScreen> with WidgetsBindingObserver {
  ...
  PlayerTimerType _getTimerType(GameState state) {
    ...
    if (subPhase == SubPhase.sheriffLook || subPhase == SubPhase.sheriffCheck) {
      ...
      if (currentSpeaker == sheriff.seatNumber) {
        return subPhase == SubPhase.sheriffLook ? PlayerTimerType.seconds20 : PlayerTimerType.seconds10;
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    AppLogger.d('BUILD: isVotingActive=${gameState.isVotingActive}, voteController=${gameState.voteController != null}');
    if (gameState.isVotingActive && gameState.voteController != null) {
      _showVoteBottomSheet(gameState.voteController!);
    }
    
    AppLogger.d('BUILD gameScreen: currentSpeakerSeat=${gameState.currentSpeakerSeat}');
        ...
        body: Padding(
          ...
          child: Column(
            children: [
              ...
              BottomControls(
                onBack: () {
                  if (gameState.currentSubPhase == SubPhase.speeches) {
                    _vm.previousSpeaker();
                  } else {
                    _vm.onPhaseBack();
                  }
