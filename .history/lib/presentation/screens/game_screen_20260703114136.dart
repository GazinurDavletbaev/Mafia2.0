...
void initState() {
  ...
  WidgetsBinding.instance.addPostFrameCallback((_) {
    ...
    // ✅ Проверяем, есть ли имена в initialData
    final names = widget.initialData.playerNames;
    
    if (!hasNames && names.isNotEmpty) {
      _vm.initializeGame(
        playerNames: names,
      ...
      // ✅ Если имена уже есть в gameState, но playerNames не синхронизированы
      // обновляем playerNames из gameState
      final currentNames = players.map((p) => p.name).toList();
      if (currentNames.isNotEmpty && currentNames != widget.initialData.playerNames) {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              _dialogShown = false;
              _notifyGameStateChanged();
              widget.onSwitchToTab(4); // переключаемся на вкладку "Протокол"
            },
            child: const Text(
              '📋 ПРОТОКОЛ ИГРЫ',
