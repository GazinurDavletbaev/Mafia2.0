...
class GameProtocolScreen extends StatelessWidget {
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSection(
              'Раздача ролей',
              gameState.players
                  .map((p) => 'Место ${p.seatNumber}: ${_getRoleName(p.role)}')
                  .toList()),

          // Ночные действия из nightActions
          _buildSection('Ночные действия',
              _buildNightActions(gameState.nightActions ?? [])),

          // Голосования (из voteHistory)
          _buildSection(
              'Голосования', _buildVoteHistory(gameState.voteHistory)),
  ...
  List<String> _buildNightActions(List<int> nightActions) {
    ...
    for (int i = 0; i < nightActions.length; i += 3) {
      ...
      result.add(
          'Ночь $nightNum: убийство ${kill == 0 ? "промах" : "игрок $kill"}, дон → $donCheck, шериф → $sheriffCheck');
    }
    return result;
  }
