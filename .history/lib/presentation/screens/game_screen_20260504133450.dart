...
class _GameContent extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // Временное решение: создаём ViewModel напрямую
    final viewModel = ref.watch(gameViewModelFamily(gameId));
    
    // Оборачиваем в ConsumerStatefulWidget чтобы держать ViewModel
    return _GameContentBody(viewModel: viewModel);
  }
}

class _GameContentBody extends ConsumerStatefulWidget {
  final GameViewModel viewModel;
  
  const _GameContentBody({required this.viewModel});

  @override
  ConsumerState<_GameContentBody> createState() => _GameContentBodyState();
}

class _GameContentBodyState extends ConsumerState<_GameContentBody> {
  late final GameViewModel vm;
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _showSettingsMenu(context, vm),
          ),
        ],
      ),
      body: Column(
  ...
  void _showSettingsMenu(BuildContext context, GameViewModel vm) {
