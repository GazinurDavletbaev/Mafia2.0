...
class _GameContent extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final GameViewModel vm = ref.watch(gameViewModelProvider(gameId));
    final currentState = vm.state;  // ← переименовал в currentState
    final players = currentState.players;

    return Scaffold(
      appBar: AppBar(
        title: Text('День ${currentState.currentDay}'),
        actions: [
          IconButton(
      ...
      body: Column(
        children: [
          PhaseHeader(
            ...
            subPhaseIndex: currentState.currentSubPhaseIndex,
          ),
          Expanded(
            child: PlayerGrid(
              players: players,
              currentSpeaker: currentState.currentSpeakerSeat,
