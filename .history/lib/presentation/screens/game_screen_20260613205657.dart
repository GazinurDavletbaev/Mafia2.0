...
class _GameScreenState extends ConsumerState<GameScreen> {
  ...
  void _markPlayerSkippedSpeech(int seatNumber) {
    final players = _vm.state.players;
    final index = players.indexWhere((p) => p.seatNumber == seatNumber);
    if (index == -1) return;

    final updatedPlayer = players[index].copyWith(hasSkippedSpeech: true);
  ...
  PlayerTimerType _getTimerType(GameState state) {
    ...
    if (subPhase == SubPhase.speeches && currentSpeaker != null) {
      ...
      if (!player.hasSkippedSpeech && player.fouls == 3) {
        _markPlayerSkippedSpeech(currentSpeaker!);
