...
class _ClubScreenState extends ConsumerState<ClubScreen> {
  ...
  int _gamesCount = 0;
  int _currentTab = 0;
  int? _pendingRequestId; // ← ДОБАВИТЬ
  ...
  void _selectClub(int clubId) {
    setState(() {
      ...
      _members = [];
      _currentTab = 0;
      _pendingRequestId = null; // ← ДОБАВИТЬ
    });
    ...
    try {
      if (clubId != null) {
        ...
        if (result['success']) {
          ...
          await _loadGames();
          await _loadMembers();
          await _loadPendingRequestId(); // ← ДОБАВИТЬ
        } else {
          setState(() {
            _isLoading = false;
            _club = null;
  ...
  // 🔥 НОВЫЙ МЕТОД ДЛЯ ПОЛУЧЕНИЯ ID ЗАЯВКИ
  Future<void> _loadPendingRequestId() async {
    ...
    try {
      ...
      // Получаем все заявки клуба
      final requestsResult = await ClubService.getClubRequests(_club!['id']);
      if (!requestsResult['success']) return;

      final requests = requestsResult['requests'] as List? ?? [];

      // Находим свою заявку со статусом pending
      ...
      setState(() {
        _pendingRequestId = myRequest?['id'];
      });
    } catch (e) {
      print('❌ Ошибка получения ID заявки: $e');
      setState(() {
        _pendingRequestId = null;
      });
    }
          ...
          Column(
            children: [
              ClubHeader(
                ...
                pendingRequestId: _pendingRequestId, // ← ДОБАВИТЬ
              ),
              Expanded(
