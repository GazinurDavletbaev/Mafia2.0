...
class _ClubScreenState extends ConsumerState<ClubScreen> {
  ...
  int _gamesCount = 0;
  int? _selectedClubId; // ID выбранного клуба (если чужой)
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ...
        _loadAllClubs(); // ✅ загружаем клубы один раз
      }
    });
  }
  ...
  void _selectClub(int clubId) {
    ...
    _loadData(clubId: clubId); // ✅ загружаем выбранный клуб
  }
  ...
  print('📦 _loadData START, clubId = $clubId');
  print('📦 _loadData: текущий _club = ${_club?['title']}');
  
  setState(() => _isLoading = true);

  if (clubId != null) {
    print('📦 Загружаем выбранный клуб с сервера: $clubId');
    ...
    if (result['success']) {
      ...
      print('📦 _club присвоен: ${_club?['title']}');
      
      await _loadRating();
      
      setState(() {});
      print('📦 _club после setState: ${_club?['title']}');
    } else {
      print('❌ Ошибка загрузки клуба: ${result['error']}');
    }
  } else {
    print('📦 Загружаем свой клуб');
    ...
    if (myClubsResult['success'] && clubData != null && clubData['id'] != null) {
      ...
      print('📦 Свой клуб загружен: ${_club?['title']}');
      await _loadRating();
    } else {
      print('❌ НЕТ КЛУБА!');
      _hasClub = false;
  ...
  print('📦 _loadData END, _club = ${_club?['title']}');
}
  ...
  Widget _buildClubContent(ThemeData theme, bool isDark) {
    ...
    print('📦 _buildClubContent _club: ${_club?['title']}'); // ← добавить
