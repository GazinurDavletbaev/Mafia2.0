...
class _SeatSetupScreenState extends ConsumerState<SeatSetupScreen> {
  ...
  void _initControllers() {
    print('📦 _initControllers: playerNames=${widget.initialData.playerNames}');
  ...
  @override
  void didUpdateWidget(covariant SeatSetupScreen oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (oldWidget.initialData.playerNames != widget.initialData.playerNames) {
      print('📦 didUpdateWidget: обновляем контроллеры');
      _updateControllers();
    }
  }
  ...
  Widget _buildTextFieldWithSearch(int index, bool isLeft) {
    ...
    return Autocomplete<Map<String, dynamic>>(
        ...
        return TextField(
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: _selectedPlayers[index] != null
                ? CircleAvatar(
                    radius: 14,
                    backgroundImage: _selectedPlayers[index]?['avatar_url'] != null &&
                            _selectedPlayers[index]!['avatar_url'].toString().isNotEmpty
                        ? NetworkImage(_selectedPlayers[index]!['avatar_url'])
                        : null,
                    child: _selectedPlayers[index]?['avatar_url'] == null ||
                            _selectedPlayers[index]!['avatar_url'].toString().isEmpty
                        ? Image.asset(
                  ...
                  return ListTile(
                    leading: CircleAvatar(
                      ...
                      backgroundImage: member['avatar_url'] != null &&
                              member['avatar_url'].toString().isNotEmpty
                          ? NetworkImage(member['avatar_url'])
                          : null,
                      child: member['avatar_url'] == null ||
                              member['avatar_url'].toString().isEmpty
                          ? Image.asset(
                              'assets/mafia_logo.png',
                              width: 20,
                              height: 20,
                    ...
                    onTap: () {
                      print('✅ Выбран игрок: ${member['username']}');
                      setState(() {
      ...
      onSelected: (Map<String, dynamic> selection) {
        print('✅ onSelected: ${selection['username']}');
      },
    );
  }
}