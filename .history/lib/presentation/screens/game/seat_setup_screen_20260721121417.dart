...
class _SeatSetupScreenState extends ConsumerState<SeatSetupScreen> {
  ...
  final updatedPlayers = widget.initialData.gameState.players.asMap().entries.map((entry) {
    ...
    final int? userId = selected?['id'] as int?;
    final String? avatarUrl = (userId != null) ? selected?['avatar_url'] as String? : '';

    print('📦 _notifyChanges: index=$index, name=$newName, avatarUrl=$avatarUrl, userId=$userId');
