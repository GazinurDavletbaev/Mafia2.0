...
class _GameProtocolViewScreenState
  ...
  List<double> _bonusPoints = List.generate(10, (_) => 0.0);
  List<String> _notes = [];
  String _protestComment = '';
  ...
  GameState _convertToGameState(Map<String, dynamic> data) {
    ...
    for (final p in playersData) {
      ...
      if (seat >= 1 && seat <= 10) {
        _bonusPoints[seat - 1] = bonus;
      }
    }

    // 🔥 ПАРСИМ ПОЯСНЕНИЯ
    _notes = (data['notes'] as List? ?? []).map((e) => e.toString()).toList();
    _protestComment = data['protest_comment'] ?? '';
    ...
    for (final entry in voteHistoryData.entries) {
      ...
      final result = (dayData['result'] as List? ?? []).map((e) => e as int).toList();
  ...
  Widget _buildProtocolContent() {
    // 🔥 СОЗДАЁМ КОНТРОЛЛЕРЫ ДЛЯ ПОЯСНЕНИЙ
    final noteControllers = List.generate(
      10,
      (i) => TextEditingController(text: i < _notes.length ? _notes[i] : ''),
    );
    final protestCommentController = TextEditingController(text: _protestComment);
      ...
      child: Column(
        children: [
          ...
          ProtocolNotes(
            ...
            protestCommentController: protestCommentController,
          ),
        ],
      ),
    );
