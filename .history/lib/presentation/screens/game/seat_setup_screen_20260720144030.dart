...
class _SeatSetupScreenState extends ConsumerState<SeatSetupScreen> {
  ...
  void initState() {
    ...
    _selectedPlayers = List.generate(10, (index) => null); // ✅ ВСЕГДА 10
  ...
  void _notifyChanges() {
    ...
    final emptyFields = names.where((name) => name.isEmpty).length;
  if (emptyFields > 0) {
    print('⚠️ Не все игроки заполнены! Пустых полей: $emptyFields');
    
    // ✅ Показываем уведомление
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('⚠️ Заполните всех 10 игроков! (пустых: $emptyFields)'),
    ...
    final avatars = List.generate(10, (index) {
      if (_selectedPlayers.length > index && _selectedPlayers[index] != null) {
        return _selectedPlayers[index]?['avatar_url'];
      }
      return null;
    });
