...
class _SeatSetupScreenState extends ConsumerState<SeatSetupScreen> {
  ...
  void _notifyChanges() {
    ...
    final updatedPlayers = widget.initialData.gameState.players.asMap().entries.map((entry) {
      ...
      return player.copyWith(name: name, userId: userId);
    }).toList();
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ Восстанавливаем имена при каждом перестроении
    for (int i = 0; i < 10; i++) {
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                Expanded(child: _buildTextFieldWithSearch(index, isLeft)),
              ],
            ),
          ),
        );
  ...
  Widget _buildTextFieldWithSearch(int index, bool isLeft) {
    ...
    return Autocomplete<Map<String, dynamic>>(
      ...
      displayStringForOption: (Map<String, dynamic> option) => option['username'] ?? '',
      fieldViewBuilder: (context, textEditingController, focusNode, onFieldSubmitted) {
        // ✅ Связываем контроллеры — один источник правды
            ...
            suffixIcon: _selectedPlayers[index] != null
                ? CircleAvatar(
                    ...
                    child: _selectedPlayers[index]?['avatar_url'] == null
                        ? Image.asset('assets/mafia_logo.png', width: 20, height: 20, fit: BoxFit.contain)
                        : null,
                  )
                : null,
          ),
