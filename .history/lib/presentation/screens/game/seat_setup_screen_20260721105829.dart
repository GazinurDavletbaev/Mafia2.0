...
class _SeatSetupScreenState extends ConsumerState<SeatSetupScreen> {
  ...
  final updatedPlayers =
    ...
    // ✅ Если имя не изменилось — сохраняем старые данные (или из _selectedPlayers)
    final selected = _selectedPlayers.length > index ? _selectedPlayers[index] : null;
    return player.copyWith(
      name: newName,
      avatarUrl: selected?['avatar_url'] ?? player.avatarUrl,
      userId: selected?['id'] ?? player.userId,
    );
  }).toList();
