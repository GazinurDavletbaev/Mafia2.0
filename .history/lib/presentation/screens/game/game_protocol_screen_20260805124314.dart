...
class _GameProtocolScreenState extends ConsumerState<GameProtocolScreen> {
  ...
  void initState() {
    ...
    _tournamentController.text = widget.gameState.tournamentName ?? 'РЕЙТИНГ';
  ...
  Widget _buildBonusPointsCell(int index) {
    ...
    final hasPpk = widget.gameState.ppkPlayerSeat == player.seatNumber;
          ...
          children: [
            ...
            _infoRow(
              ...
              userAsync.when(
                  data: (user) =>
                      user?['username'] ??
                      'Неизвестен', // ← если null → 'Неизвестен'
                  loading: () => 'loading',
  ...
  void _saveProtocol() async {
    ...
    final data = {
      ...
      'players': widget.gameState.players.map((p) {
        ...
        final hasPpk = widget.gameState.ppkPlayerSeat == p.seatNumber;

        // 🔥 БОНУС: -0.5 если удалён, -1 если ППК, иначе из _bonusPoints
        double bonus;
        if (hasPpk) {
          bonus = -1.0;
        } else if (isRemoved) {
