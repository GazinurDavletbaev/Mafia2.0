...
class _SeatSetupScreenState extends ConsumerState<SeatSetupScreen> {
  ...
  Future<void> _loadClubMembers() async {
    ...
    if (club != null) {
      ...
      print('📦 getClubMembers result: $result'); // ✅ ДОБАВЬ

      if (result['success']) {
        _clubMembers =
            (result['members'] as List? ?? []).cast<Map<String, dynamic>>();
        print('📦 _clubMembers: $_clubMembers'); // ✅ ДОБАВЬ
      }
    }
  ...
  void _notifyChanges() {
    ...
    // Обновляем gameState с новыми именами и userId
  ...
  Widget _buildTextFieldWithSearch(int index, bool isLeft) {
    ...
    print('🔍 optionsBuilder вызван: "${textEditingValue.text}"'); // ✅
    if (textEditingValue.text.isEmpty) {
      print('🔍 Пустой запрос -> возвращаем пустой список');
  ...
  optionsViewBuilder: (context, onSelected, options) {
    print('📦 optionsViewBuilder вызван, options: ${options.length}'); // ✅
      },
      fieldViewBuilder:
            ...
            child: Container(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, i) {
                  final member = options.elementAt(i);
                  return ListTile(
      ...
      onSelected: (Map<String, dynamic> selection) {
        // ✅ Дополнительная обработка при выборе
