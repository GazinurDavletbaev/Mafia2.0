  ...
  Future<void> _loadData() async {
    ...
    if (clubResult['success'] && clubResult['club'] != null) {
      ...
      if (membersResult['success']) {
        _members = (membersResult['members'] as List? ?? []).cast<Map<String, dynamic>>();
_judges = (membersResult['judges'] as List? ?? []).cast<Map<String, dynamic>>();
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_club != null ? 'Участники "${_club!['title']}"' : 'Участники'),
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              // TODO: открыть поиск пользователей для добавления
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Поиск пользователей для добавления (в разработке)'),
                  backgroundColor: Colors.orange,
                ),
              );
            },
    ...
    return Card(
      ...
      child: ListTile(
        ...
        leading: CircleAvatar(
          backgroundColor: isPresident ? Colors.orange : Colors.grey.shade300,
          child: Text(
  ...
  Widget _buildJudgeCard(Map<String, dynamic> judge, bool isDark) {
    ...
    return Card(
      ...
      child: ListTile(
        contentPadding: const EdgeInsets.all(16),
        leading: CircleAvatar(
          backgroundColor: isPresident ? Colors.orange : Colors.blue,
          child: Text(
        ...
        trailing: isPresident
            ? null
            : IconButton(
                icon: const Icon(Icons.gavel_off, color: Colors.orange),
                onPressed: () async {
                  final confirm = await _showConfirmDialog(
                    context,
