...
import '../widgets/phase_button.dart';  // ← ДОБАВИТЬ ЭТОТ ИМПОРТ
...
class _GameContent extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final vm = ref.watch(gameViewModelProvider(gameId));
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          PhaseButton(
            onBack: vm.onPhaseBack,      // ← должно быть в GameViewModel
            onForward: vm.onPhaseForward, // ← должно быть в GameViewModel
          ),
        ],
      ),
    );
