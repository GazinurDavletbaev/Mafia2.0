...
class _LobbyScreenState extends ConsumerState<LobbyScreen> {
  ...
  // 🔥 ЛОГИКА ПЕРЕКЛЮЧЕНИЯ ВКЛАДОК С ТАЙМЕРОМ
  ...
  print('🔍 _onTabChanged: wasOnGame=$wasOnGame, isNowOnGame=$isNowOnGame');
  print('🔍 timerState: isRunning=${timerState.isRunning}, remaining=${timerState.remainingSeconds}');

  if (wasOnGame && !isNowOnGame) {
    print('🔍 Выходим с GameScreen, показываем глобальный таймер');
    if (timerState.isRunning && timerState.remainingSeconds > 0) {
      ref.read(globalTimerVisibleProvider.notifier).state = true;
      print('🔍 globalTimerVisible = true');
    }
  }

  if (isNowOnGame) {
    print('🔍 Возвращаемся на GameScreen, скрываем глобальный таймер');
    ref.read(globalTimerVisibleProvider.notifier).state = false;
    
    if (timerState.isRunning) {
      timerNotifier.syncFromGlobal(timerState.remainingSeconds);
      print('🔍 Синхронизировали таймер: ${timerState.remainingSeconds}');
    }
  }
