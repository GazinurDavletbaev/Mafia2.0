...
class _LobbyScreenState extends ConsumerState<LobbyScreen> {
  ...
  // 🔥 ЗАПУСК ТАЙМЕРА НАВБАРА
  void _startNavBarTimer(int seconds) {
    _navBarTimerSeconds = seconds;
    _navBarTimer?.cancel();
    _navBarTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (_navBarTimerSeconds != null && _navBarTimerSeconds! > 0) {
        setState(() {
          _navBarTimerSeconds = _navBarTimerSeconds! - 1;
        });
        // 🔥 ОБНОВЛЯЕМ ПРОВАЙДЕР
        ref.read(navBarTimerProvider.notifier).state = _navBarTimerSeconds;
      } else {
        _stopNavBarTimer();
      }
    });
  ...
  // 🔥 ОСТАНОВКА ТАЙМЕРА НАВБАРА
  void _stopNavBarTimer() {
    _navBarTimer?.cancel();
    setState(() {
      _navBarTimerSeconds = null;
    });
    ref.read(navBarTimerProvider.notifier).state = null;
  }

  // 🔥 ПЕРЕКЛЮЧЕНИЕ ВКЛАДОК
  void _onTabChanged(int index) {
    final wasOnGame = _selectedIndex == 2;
    final isNowOnGame = index == 2;
    final timerState = ref.read(timerProvider);

    // 🔥 ВЫХОДИМ С ЭКРАНА ИГРЫ — ЗАПУСКАЕМ ТАЙМЕР НАВБАРА
    if (wasOnGame && !isNowOnGame) {
      if (timerState.isRunning && timerState.remainingSeconds > 0) {
        _startNavBarTimer(timerState.remainingSeconds);
      }
    }

    // 🔥 ВОЗВРАЩАЕМСЯ НА ЭКРАН ИГРЫ — СИНХРОНИЗИРУЕМ
    if (isNowOnGame) {
      if (_navBarTimerSeconds != null && _navBarTimerSeconds! > 0) {
        final vm = ref.read(gameViewModelProvider.notifier);
        vm.syncTimerFromGlobal(_navBarTimerSeconds!);
      }
      _stopNavBarTimer();
    }
