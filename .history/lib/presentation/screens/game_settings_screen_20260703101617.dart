...
import 'package:mafia_help/presentation/screens/seat_setup_screen.dart';
...
class GameSettingsScreen extends ConsumerStatefulWidget {
  ...
  const GameSettingsScreen({super.key, this.onSettingsSaved});
  ...
  void _nextStep() {
    if (_formKey.currentState!.validate()) {
      if (widget.onSettingsSaved != null) {
        // Передаём данные в LobbyScreen и переключаемся на вкладку "Игроки"
        widget.onSettingsSaved!(
          int.parse(_tableController.text),
          int.parse(_gameController.text),
          _selectedDate,
          _judgeController.text,
          _tournamentController.text,
          _stageController.text,
        );
      } else {
        // Старое поведение (если вызывается отдельно)
        Navigator.push(
          context,
          MaterialPageRoute(
