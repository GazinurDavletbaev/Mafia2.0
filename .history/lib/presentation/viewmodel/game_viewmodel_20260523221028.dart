  ...
  Future<void> onPhaseForward() async {
    ...
    switch (state.currentSubPhase) {
      ...
      case SubPhase.revote:
        AppLogger.d(
          'Voting phase started, candidates: ${state.nominatedSeats}, ${state.tiedSeats}',
        );
      ...
      case SubPhase.finalWord:
        ...
        if (currentSeat != null && tiedSeats.isNotEmpty) {
          ...
          if (nextIndex < tiedSeats.length) {
            // Следующий лидер → сразу даём ему минуту, без дополнительного нажатия
            state = state.copyWith(currentSpeakerSeat: tiedSeats[nextIndex]);
            // Не выходим, продолжаем обработку
            break;
          } else {
            // Все выбыли → ночь
            state = state.copyWith(
          ...
          // Обычный finalWord (не из eliminationVote) → ночь через calculateNextState
          state = await _phase.calculateNextState(state);
        }
        break;
