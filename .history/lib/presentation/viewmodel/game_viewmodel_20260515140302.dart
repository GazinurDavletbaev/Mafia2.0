...
class GameViewModel extends StateNotifier<GameState> {
  ...
  Future<void> onPhaseBack() async {
    ...
    if (previousSpeaker == lastSavedSpeaker) {
      AppLogger.d(
        'onPhaseBack: previousSpeaker matches lastSavedSpeaker, restoring last saved state without pop',
      );
      state = lastSavedState;
      AppLogger.d(
        'onPhaseBack: new state speaker = ${state.currentSpeakerSeat}',
      );
      return;
    }
    ...
    state = previousState;
    AppLogger.d(
      'onPhaseBack: new state speaker = ${state.currentSpeakerSeat}, history length = ${_history.length}',
    );
  }
