...
class SeatSetupNotifier extends StateNotifier<SeatSetupState> {
  ...
  void updateFilteredList() {
    ...
    for (int i = 0; i < state.selectedPlayers.length; i++) {
      ...
      if (selected != null) {
        takenUsernames.add(selected['username']);
      }
    }

    // 🔥 ДОБАВЛЯЕМ ТЕКУЩИЙ ТЕКСТ В takenUsernames
    // чтобы не показывать его в списке
    final currentText = state.nameControllers[state.focusedIndex].text.trim();
    if (currentText.isNotEmpty) {
      takenUsernames.add(currentText);
    }
