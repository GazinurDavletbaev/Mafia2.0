...
class GameViewModel extends StateNotifier<GameState> {
  ...
  TimerController? _timerController; // 🔥 ДОБАВИЛИ
  Timer? _globalTimer; // 🔥 ДОБАВИЛИ
  ...
  void startTimer(int seconds) {
    _timerController = TimerController(
      ...
      onTick: () {
        // Обновляем состояние при тике
        final remaining = _timerController?.remaining ?? 0;
        if (remaining > 0) {
          // Можно обновить UI через состояние
        }
      },
      onComplete: () {
  ...
  bool isTimerRunning() {
    return _timerController != null && _timerController!.remaining > 0;
  }
