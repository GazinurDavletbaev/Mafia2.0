...
class VoteCalculatorActions {
  ...
  if (isRevote) {
    // Обновляем существующий день — добавляем раунд
  ...
  break;
        ...
        case VoteResultType.tieBreak:
          // Это ПЕРЕСТРЕЛКА (ничья) — кандидаты говорят 30 секунд
          final voteDay = VoteDay(
            vote: votes, // ← голоса основного голосования
            revote: null, // ← пока нет переголосования
            eliminated: false,
            eliminationVotes: 0,
            result: [],
          );
          ...
          newState = _vm.state.copyWith(
            currentSubPhase: SubPhase.tieBreak, // ← перестрелка
