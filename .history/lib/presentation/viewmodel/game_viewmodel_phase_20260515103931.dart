...
class PhaseActions {
  ...
  Future<void> onPhaseForward() async {
    ...
    // В PhaseActions.onPhaseForward() при переходе на speeches
    ...
    if (nextPhase == SubPhase.speeches) {
      ...
      if (allAlive.isNotEmpty) {
        // Берём первого говорящего из предыдущего дня
        final previousFirstSpeaker = _vm.state.dayStarterSeat;
        int firstSpeaker;

        if (previousFirstSpeaker != null) {
          // Ищем следующего живого после previousFirstSpeaker
          final index = allAlive.indexOf(previousFirstSpeaker);
          if (index >= 0 && index + 1 < allAlive.length) {
            firstSpeaker = allAlive[index + 1];
          } else {
            firstSpeaker =
                allAlive[0]; // если последний в списке → начинаем с первого
          }
          AppLogger.d('previousFirstSpeaker = $previousFirstSpeaker');
          AppLogger.d('allAlive = $allAlive');
          AppLogger.d('index = $index');
          AppLogger.d('firstSpeaker = $firstSpeaker');
        } else {
          // Первый день → начинаем с первого живого
          firstSpeaker = allAlive.first;
        }
