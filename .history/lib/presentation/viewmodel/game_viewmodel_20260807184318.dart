...
import 'package:mafia_help/presentation/widgets/timer/timer_controller.dart';
...
class GameViewModel extends StateNotifier<GameState> {
  ...
  TimerController? _timerController; // 🔥 ДОБАВИТЬ
  ...
  void initializeGame({
    ...
    List<int?>? userIds, // ✅ массив userId
    List<String?>? avatars, // ✅ массив аватарок
    int? tableNumber,
    int? gameNumber,
    DateTime? gameDate,
    String? judgeName,
    ...
    final newPlayers = List.generate(10, (index) {
      final seat = index + 1;
      final name = playerNames.length > index ? playerNames[index] : '';
      final userId =
          userIds != null && userIds.length > index ? userIds[index] : null;
      final avatar =
          avatars != null && avatars.length > index ? avatars[index] : null;
      ...
      return PlayerModel(
        ...
        userId: userId, // ✅
        avatarUrl: avatars != null && avatars.length > index
            ? avatars[index]
            : null, // ✅
      );
    });
  ...
  void submitNightAction(int value) {
    ...
    switch (subPhase) {
      case SubPhase.mafiaShoot:
        index = (state.currentDay - 1) * 3 + 0;
        break;
      case SubPhase.donCheck:
        index = (state.currentDay - 1) * 3 + 1;
        break;
      case SubPhase.sheriffCheck:
        index = (state.currentDay - 1) * 3 + 2;
        break;
      default:
        return;
    }
    ...
    print(index);
    // ✅ Если уже есть значение — игнорируем
    if (index < currentActions.length) {
      AppLogger.d(
          'submitNightAction: уже введено значение ${currentActions[index]}');
      return;
    }
...
// lib/presentation/viewmodel/game_viewmodel.dart
  ...
  void resetGame() {
    ...
    _ref.read(navBarTimerProvider.notifier).state = null;
  ...
  void syncTimerFromGlobal(int seconds) {
    if (_timerController != null) {
      _timerController!.sync(seconds);
      // Обновляем провайдер
      _ref.read(navBarTimerProvider.notifier).state = seconds;
    }
  }

  // 🔥 ЗАПУСК ТАЙМЕРА
  void startTimer(int seconds) {
    print('🔥🔥🔥 startTimer called with seconds: $seconds');

    _ref.read(navBarTimerProvider.notifier).state = seconds;
    print('🔥 navBarTimer set to: $seconds');

    _timerController = TimerController(
      totalSeconds: seconds,
      onTick: () {
        final remaining = _timerController!.remaining;
        print('🔥 tick: remaining = $remaining');
        _ref.read(navBarTimerProvider.notifier).state = remaining;
      },
      onComplete: () {
  ...
  int getRemainingSeconds() {
    return _timerController?.remaining ?? 0;
  }

  // 🔥 ПРОВЕРКА, БЕЖИТ ЛИ ТАЙМЕР
  bool isTimerRunning() {
    return _timerController != null && _timerController!.isActive;
  }
}
