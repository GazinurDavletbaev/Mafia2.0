...
class GameViewModel extends StateNotifier<GameState> {
  ...
  AppLogger.d('Voting started, candidates: $candidates');
  
  if (candidates.isEmpty) {
    AppLogger.d('No candidates, skipping voting');
    state = await _phase.calculateNextState(state);
    break;
  }
  ...
  state = state.copyWith(
    ...
    currentSpeakerSeat: candidates[0],
    isVotingActive: true,
    voteController: VoteController([candidates[0]]),
  );
  break;
