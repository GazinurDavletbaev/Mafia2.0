  ...
  Future<void> onPhaseForward() async {
    ...
    switch (state.currentSubPhase) {
      ...
      case SubPhase.tieBreak:
        await _speeches.nextSpeakerForTieBreak(); // новый метод
        _history.push(state);
        break;
      ...
      case SubPhase.revote:
        // Для revote берём кандидатов из tiedSeats (текущие лидеры после перестрелки)
        final candidates = state.tiedSeats.isNotEmpty
            ? state.tiedSeats
            : state.nominatedSeats;
        AppLogger.d('Revote phase started, candidates: $candidates');
        state = state.copyWith(
          isVotingActive: true,
          nominatedSeats: candidates, // ← обновляем nominatedSeats
          tiedSeats: [], // ← очищаем tiedSeats
      ...
      case SubPhase.finalWord:
        ...
        // Проверяем, есть ли ещё лидеры в tiedSeats
        final tiedSeats = state.tiedSeats;
        final currentSeat = state.currentSpeakerSeat;

        if (currentSeat != null && tiedSeats.isNotEmpty) {
          final currentIndex = tiedSeats.indexOf(currentSeat);
          final nextIndex = currentIndex + 1;

          if (nextIndex < tiedSeats.length) {
            // Следующий лидер
            state = state.copyWith(currentSpeakerSeat: tiedSeats[nextIndex]);
            // НЕ выходим, продолжаем - сразу переходим к следующему
            return; // важно: return, чтобы не идти дальше
          } else {
            // Все выбыли → ночь
            state = state.copyWith(
          ...
          // Обычный finalWord → ночь через calculateNextState
          state = await _phase.calculateNextState(state);
        }
        break;
