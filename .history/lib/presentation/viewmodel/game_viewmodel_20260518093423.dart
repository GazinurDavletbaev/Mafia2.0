...
class GameViewModel extends StateNotifier<GameState> {
  ...
  Future<void> onPhaseForward() async {
    ...
    final phaseNamess = _history.states
        .map((s) => s.currentSubPhase.name)
        .toList();
    AppLogger.d('history phases: $phaseNamess');
  ...
  void pushCurrentStateToHistory() {
    if (_history.states.last.currentSubPhase == SubPhase.roleDistribution) {
      return;
    }
    _history.push(state);
    AppLogger.d(
      'pushCurrentStateToHistory: speaker ${state.currentSpeakerSeat}',
    );
  }
