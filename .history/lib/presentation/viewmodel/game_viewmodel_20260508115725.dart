...
class GameViewModel extends StateNotifier<GameState> {
  ...
  Future<void> onSpeechTimerComplete() async {
    ...
    final currentIndex = alivePlayers.indexOf(currentSeat);
    
    if (currentIndex != -1 && currentIndex + 1 < alivePlayers.length) {
      final nextSeat = alivePlayers[currentIndex + 1];
  ...
  Future<void> nextSpeaker() async {
    ...
    if (currentIndex != -1 && currentIndex + 1 < aliveSeats.length) {
      ...
      // Речи закончены → переходим к голосованию
      await onPhaseForward();
    }
  }
