...
/ ✅ Проверяем, не вводили ли уже значение в этой фазе
  int index = 0;
  switch (subPhase) {
    case SubPhase.mafiaShoot:
      index = state.currentDay * 3 + 0;
      break;
    case SubPhase.donCheck:
      index = state.currentDay * 3 + 1;
      break;
    case SubPhase.sheriffCheck:
      index = state.currentDay * 3 + 2;
      break;
    default:
      return;
  }
  ...
  if (index < currentActions.length && currentActions[index] != 0 && currentActions[index] != -1) {
    AppLogger.d('submitNightAction: уже введено значение ${currentActions[index]}');
    return;
  }
