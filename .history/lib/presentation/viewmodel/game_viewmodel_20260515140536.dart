...
class GameViewModel extends StateNotifier<GameState> {
  ...
  AppLogger.d('onPhaseBack: currentSpeaker = $currentSpeaker');
  
  final lastSavedState = _history.last;
  final lastSavedSpeaker = lastSavedState.currentSpeakerSeat;
  AppLogger.d('onPhaseBack: lastSavedSpeaker = $lastSavedSpeaker');
  ...
  // Если предыдущий говорящий совпадает с последним сохранённым
  if (previousSpeaker == lastSavedSpeaker) {
    AppLogger.d('onPhaseBack: previousSpeaker matches lastSavedSpeaker, restoring last saved state without pop');
    state = lastSavedState;
    AppLogger.d('onPhaseBack: new state speaker = ${state.currentSpeakerSeat}');
    return;
  }
  
  AppLogger.d('onPhaseBack: no match, popping last state');
  final previousState = _history.pop();
  state = previousState;
  AppLogger.d('onPhaseBack: new state speaker = ${state.currentSpeakerSeat}, history length = ${_history.length}');
}
