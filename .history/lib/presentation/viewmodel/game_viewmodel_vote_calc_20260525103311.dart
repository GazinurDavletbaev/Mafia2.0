  ...
  void _finalizeVoting(Map<int, int> votes) {
    ...
    switch (result.type) {
      ...
      case VoteResultType.tieBreak:
        ...
        final currentTiedCount = result.seats.length;

        if (previousTiedCount > 0 && currentTiedCount < previousTiedCount) {
          // Количество лидеров уменьшилось → ещё одна перестрелка
          newState = _vm.state.copyWith(
            currentSubPhase: SubPhase.tieBreak,
            tiedSeats: result.seats,
            currentTieIndex: 0,
            currentSpeakerSeat: result.seats.isNotEmpty
                ? result.seats[0]
                : null,
            voteController: null,
            isVotingActive: false,
          );
        } else if (previousTiedCount == currentTiedCount) {
          // Количество не изменилось → голосование за подъём
          newState = _vm.state.copyWith(
          ...
          // Обычный случай (первая перестрелка)
          newState = _vm.state.copyWith(
            currentSubPhase: SubPhase.tieBreak,
            tiedSeats: result.seats,
            currentTieIndex: 0,
