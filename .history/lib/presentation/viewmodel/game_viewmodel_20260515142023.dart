...
class GameViewModel extends StateNotifier<GameState> {
  ...
  // Если текущий говорящий совпадает с последним сохранённым
  if (currentSpeaker == lastSavedSpeaker) {
    AppLogger.d('onPhaseBack: speakers match, restoring last saved state without pop');
    state = lastSavedState;
    AppLogger.d('onPhaseBack: new state speaker = ${state.currentSpeakerSeat}');
    return;
  }
  
  AppLogger.d('onPhaseBack: speakers differ, popping last state');
