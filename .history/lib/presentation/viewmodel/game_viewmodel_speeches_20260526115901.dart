...
class SpeechesActions {
  ...
  Future<void> nextSpeakerForTieBreak() async {
    ...
    if (currentIndex + 1 < tiedSeats.length) {
      ...
      // Все выступили → переходим в revote ТОЛЬКО с текущими лидерами
      _vm.state = _vm.state.copyWith(
        currentSubPhase: SubPhase.revote,
        currentSpeakerSeat: null,
        nominatedSeats: tiedSeats, // ← только лидеры
        currentTieIndex: 0,
        voteController: VoteController(tiedSeats), // ← создаём контроллер
        isVotingActive: true,
      );
    }
  }
    ...
    if (nextSpeaker != null) {
      ...
      if (candidates.isEmpty) {
        ...
        _vm.state = _vm.state.copyWith(
          ...
          speechHistory: [], // ← очищаем историю речей
        );
