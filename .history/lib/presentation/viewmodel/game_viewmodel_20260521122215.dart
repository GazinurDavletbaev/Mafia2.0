...
class GameViewModel extends StateNotifier<GameState> {
  ...
  void submitNightAction(int value) {
    ...
    // Добавляем действие в список ночных действий
    final newActions = List<int>.from(state.nightActions)..add(value);
    state = state.copyWith(nightActions: newActions);

    if (subPhase == SubPhase.mafiaShoot) {
      // value: 0 = промах, 1-10 = номер убитого
      final killedSeat = value == 0 ? null : value;
      state = state.copyWith(hasKillInLastNight: killedSeat != null);

      if (killedSeat != null) {
        final usecase = _ref.read(killPlayerUsecaseProvider);
        final (newPlayers, winner) = usecase.execute(state.players, killedSeat);
        state = state.copyWith(players: newPlayers);
        if (winner != null) {
          state = state.copyWith(
            isGameEnded: true,
      ...
      AppLogger.d(
        'Mafia shoot: ${killedSeat != null ? "убил $killedSeat" : "промах"}',
      );
