...
class PlayerActions {
  ...
  Future<void> onPlayerTap(int seatNumber) async {
    ...
    if (gotThirdFoul && isSpeaking) {
      ...
      final updatedPlayer = newPlayer.copyWith(hasSkippedSpeech: false);
      ...
      print('=== ON PLAYER TAP: SET HAS SKIPPED SPEECH ===');
      print('seat = $seatNumber');
      print('old fouls = ${oldPlayer.fouls}, new fouls = ${newPlayer.fouls}');
      // Таймер остаётся 60 секунд, не меняем
      return;
    }
  ...
  Future<void> _killPlayer(int seatNumber) async {
    ...
    if (isInVoting) {
      // Если в голосовании — переходим в ночь
