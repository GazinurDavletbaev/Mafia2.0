...
class PhaseActions {
  ...
  Future<GameState> calculateNextState(GameState currentState) async {
    ...
    if (phaseHistory.isEmpty) {
      nextPhase = isNight0Completed ? SubPhase.speeches : SubPhase.roleDistribution;
      ...
      if (_night0Order.contains(currentPhase)) {
        ...
        // Обработка речей - проверяем, закончились ли они
        if (currentPhase == SubPhase.speeches) {
          final allAlive = currentState.players
              ...
              .toList()
            ..sort();
          final lastSpeaker = currentState.speechHistory.isNotEmpty
              ? currentState.speechHistory.last
              : null;
          
          final allSpoke = lastSpeaker != null && lastSpeaker == allAlive.last;
          
          if (!allSpoke) {
            // Речи не закончились → остаёмся
            return currentState;
          }
          
          // Речи закончились → определяем следующую фазу по кандидатам
        ...
        // Остальные фазы дня идут по порядку
        else {
    ...
    if (nextPhase == SubPhase.contract) {
      ...
      return currentState.copyWith(
        ...
        currentDay: newPhaseHistory.where((p) => p == SubPhase.mafiaShoot).length,
    ...
    // Обычный переход
    final shouldIncrement = currentState.currentSubPhase == SubPhase.finalWord && nextPhase == SubPhase.mafiaShoot;
