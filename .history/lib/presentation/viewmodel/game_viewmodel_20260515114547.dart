...
class GameViewModel extends StateNotifier<GameState> {
  ...
  Future<void> onPhaseBack() async {
    ...
    state = previousState;
    AppLogger.d('onPhaseBack: restored previous state, subPhase = ${state.currentSubPhase}');
  }
...
void pushCurrentStateToHistory() {
  _history.push(state);
  AppLogger.d('pushCurrentStateToHistory: saved state with speaker ${state.currentSpeakerSeat}');
}
