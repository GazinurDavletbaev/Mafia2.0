...
class PhaseActions {
  ...
  Future<void> onPhaseBack() async {
    ...
    _vm.state = newState;
    AppLogger.d(
      'phaseHistory after pop = ${newPhaseHistory.map((p) => p.name).toList()}',
    );
  }
  ...
  if (phaseHistory.isEmpty) return; // на всякий случай, но теперь не должно быть
  ...
  AppLogger.d('phaseHistory = ${newPhaseHistory.map((p) => p.name).toList()}');
}
