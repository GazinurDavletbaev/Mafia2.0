...
class PhaseActions {
  ...
  // Ночь закончена - определяем следующую фазу в зависимости от убийства и дня
  ...
  if (nextDayNumber == 1 && hasKill) {
    ...
    nextPhase = SubPhase.finalWord;
    AppLogger.d('Night ended, Day $nextDayNumber with kill -> finalWord');
  } else {
    nextPhase = SubPhase.speeches;
    AppLogger.d('Night ended, Day $nextDayNumber without kill -> speeches');
  }
  
  // НЕ ДЕЛАЕМ newPhaseHistory здесь, а просто устанавливаем nextPhase
  // и продолжаем выполнение функции дальше
}
    }
