...
class GameViewModel extends StateNotifier<GameState> {
  ...
  late final VoteCalculatorActions _voteCalc = VoteCalculatorActions(this, _ref);

  Future<void> _loadSavedGame() => _stateActions.loadSavedGame();

  Future<void> onPhaseBack() => _phase.onPhaseBack();
  Future<void> onPhaseForward() => _phase.onPhaseForward();

  Future<void> nextSpeaker() => _speeches.nextSpeaker();
  Future<void> previousSpeaker() => _speeches.previousSpeaker();
  Future<void> setCurrentSpeaker(int? seatNumber) => _speeches.setCurrentSpeaker(seatNumber);

  Future<void> onPlayerTap(int seatNumber) => _player.onPlayerTap(seatNumber);
  Future<void> onPlayerLongPress(int seatNumber, int actionType) => _player.onPlayerLongPress(seatNumber, actionType);
  ...
  void setStateFromVoteCalculator(GameState newState) {
    state = newState;
  }
}