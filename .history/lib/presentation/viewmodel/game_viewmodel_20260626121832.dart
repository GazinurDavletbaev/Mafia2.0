  ...
  Future<void> onPhaseForward() async {
    ...
    switch (state.currentSubPhase) {
      ...
      case SubPhase.voting:
        ...
        AppLogger.d(
            'Voting phase started, candidates: $candidates, isDay0: $isDay0');

        // День 0: если 0 или 1 кандидат → автоматическое завершение
        if (isDay0 && (candidates.isEmpty || candidates.length == 1)) {
          AppLogger.d(
              'Day 0 with ${candidates.length} candidate(s), auto-finalizing');

          if (candidates.isEmpty) {
            // 0 кандидатов → сразу в ночь
            final aliveCount = state.players.where((p) => p.isAlive).length;
            ...
            state = state.copyWith(
              ...
              voteHistory: {
                ...state.voteHistory,
                state.currentDay: voteDay,
              },
            );
          } else {
            // 1 кандидат → все голоса уходят ему
            final aliveCount = state.players.where((p) => p.isAlive).length;
            final controller = VoteController(candidates);
            controller.setVotes(aliveCount);

            // Создаём запись в voteHistory
            final voteDay = VoteDay(
              vote: {candidates.first: aliveCount},
              revote: null,
              eliminated: true,
              eliminationVotes: 0,
        ...
        // 2+ кандидатов → обычное голосование
        state = state.copyWith(
          isVotingActive: true,
          voteController: VoteController(candidates),
        );
