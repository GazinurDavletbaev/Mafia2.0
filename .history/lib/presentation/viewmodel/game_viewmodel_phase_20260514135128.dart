...
class PhaseActions {
  ...
  Future<void> onPhaseBack() async {
    ...
    // Проверяем, нужно ли уменьшить день
    ...
    final newState = _vm.state.copyWith(
      ...
      currentDay: newDay,
      currentPhase: previousPhase == SubPhase.mafiaShoot
          ? Phase.night
          : Phase.day,
    );
  ...
  Future<void> onPhaseForward() async {
    ...
    if (nextPhase != null) {
      // Проверяем, нужно ли увеличить день
      final phaseRules = _ref.read(phaseRulesProvider);
      ...
      final newState = _vm.state.copyWith(
        ...
        currentDay: newDay,
        currentPhase: nextPhase == SubPhase.mafiaShoot
            ? Phase.night
            : Phase.day,
      );
      _vm.state = newState;
      AppLogger.d(
        'after: newState.currentSubPhase = ${newState.currentSubPhase}, day = $newDay',
      );
    } else {
      ...
      // День закончился → ночь
        ...
        _vm.state = newState;
        AppLogger.d(
          'transition to night ${_vm.state.currentDay + 1}: $nightPhase',
        );
      }
    }
  }
