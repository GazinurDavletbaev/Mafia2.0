...
class GameViewModel extends StateNotifier<GameState> {
  ...
  // Проверяем, находимся ли мы в речах
  final isInSpeeches = state.currentSubPhase == SubPhase.speeches;
  ...
  if (isInSpeeches) {
    ...
    // Если текущий говорящий совпадает с последним сохранённым — не удаляем
    if (currentSpeaker == lastSavedSpeaker) {
      if (_history.length >= 2) {
        final previousState = _history.states[_history.length - 2];
        state = previousState;
      }
      return;
    }
  ...
  // Во всех остальных случаях удаляем последнее состояние
