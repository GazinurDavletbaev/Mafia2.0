...
class PhaseActions {
  ...
  Future<void> onPhaseBack() async {
    ...
    AppLogger.d('before pop: stack length = ${phaseStack.history.length}');

    final newStack = changePhaseUsecase.previousPhase(phaseStack);
    final newState = _vm.state.copyWith(phaseStack: newStack);

    AppLogger.d('after pop: stack length = ${newStack.history.length}');
    AppLogger.d('current phase = ${newStack.current}');
