...
class GameViewModel extends StateNotifier<GameState> {
  ...
  Future<void> onPhaseBack() async {
    ...
    AppLogger.d(
      'onPhaseBack: currentPhase = $currentPhase, currentSpeaker = $currentSpeaker',
    );
    ...
    AppLogger.d(
      'onPhaseBack: lastSavedPhase = $lastSavedPhase, lastSavedSpeaker = $lastSavedSpeaker',
    );
    ...
    if (currentPhase == lastSavedPhase && previousSpeaker == lastSavedSpeaker) {
      AppLogger.d(
        'onPhaseBack: condition met (same phase and previousSpeaker matches lastSavedSpeaker), restoring last saved state without pop',
      );
