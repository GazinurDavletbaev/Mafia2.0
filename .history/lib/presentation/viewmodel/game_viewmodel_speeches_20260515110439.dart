...
class SpeechesActions {
  ...
  Future<void> nextSpeaker() async {
    ...
    if (currentIndex != -1 && currentIndex + 1 < orderedSeats.length) {
      ...
      final firstSpeakerOfDay =
          _vm.state.dayStarterSeat; // берём первого, а не последнего
      _vm.state = _vm.state.copyWith(
        speechHistory: [],
        dayStarterSeat: firstSpeakerOfDay,
      );
      await _ref
          .read(gameViewModelFamily(_vm.gameId).notifier)
          .onPhaseForward();
      AppLogger.d('day finished, lastSpeaker = $firstSpeakerOfDay');
    }
  }

  Future<void> previousSpeaker() async {
    AppLogger.d(
      'previousSpeaker() called, currentSpeaker=${_vm.state.currentSpeakerSeat}',
    );
