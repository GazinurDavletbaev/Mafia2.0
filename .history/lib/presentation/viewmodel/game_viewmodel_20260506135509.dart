...
class GameViewModel extends StateNotifier<GameState> {
  ...
  Future<void> _loadSavedGame() async {
    ...
    if (saved != null) {
      AppLogger.d('Loaded saved game: subPhase=${saved.currentSubPhase}');
      state = saved;
    } else {
      AppLogger.d('No saved game, initial state subPhase=${state.currentSubPhase}');
    }
    AppLogger.d('_loadSavedGame END: subPhase=${state.currentSubPhase}');
  }

  // Сохранение будет вызываться ТОЛЬКО при сворачивании (WidgetsBindingObserver)
  ...
  Future<void> onPlayerTap(int seatNumber) async {
    ...
    // Фаза раздачи ролей — показываем карточку роли
  ...
  Future<void> addVote(int seat, int votes) async {
    ...
    final newState = await usecase(seat, votes);
    state = newState;
  }
  ...
  // Добавить в конец класса GameViewModel:
