  ...
  Future<void> onPhaseForward() async {
    ...
    switch (state.currentSubPhase) {
      ...
      case SubPhase.finalWord:
        ...
        // Удаляем убитого из tiedSeats
        final remainingTied = state.tiedSeats
            .where((seat) => seat != playerToKill)
            .toList();

        if (remainingTied.isNotEmpty) {
          // Ещё есть игроки → следующий говорит
          state = state.copyWith(
            currentSpeakerSeat: remainingTied.first,
            tiedSeats: remainingTied,
          );
          // Не вызываем calculateNextState, остаёмся в finalWord
          break;
        } else {
          // Все сказали → ночь
          state = state.copyWith(
