...
class GameplayActions {
  ...
  Future<void> onPhaseBack() async {
    ...
    _vm.state = newState;
    AppLogger.d('onPhaseBack completed, newPhase=${_vm.state.currentPhase}, newSubPhase=${_vm.state.currentSubPhase}');
  }

  Future<void> onPhaseForward() async {
    AppLogger.d('GameplayActions.onPhaseForward() called, subPhase=${_vm.state.currentSubPhase}');
    final usecase = _ref.read(changePhaseUsecaseProvider);
    final newState = await usecase(goForward: true);
    _vm.state = newState;
    AppLogger.d('onPhaseForward completed, newPhase=${_vm.state.currentPhase}, newSubPhase=${_vm.state.currentSubPhase}, speaker=${_vm.state.currentSpeakerSeat}');
  }

  // ========== Речи ==========

  Future<void> nextSpeaker() async {
    AppLogger.d('nextSpeaker() called, currentSpeaker=${_vm.state.currentSpeakerSeat}, subPhase=${_vm.state.currentSubPhase}');
    ...
    // Получаем всех живых игроков, отсортированных по месту
    ...
    // Определяем порядок, начиная с dayStarterSeat
  ...
  Future<void> previousSpeaker() async {
    AppLogger.d('previousSpeaker() called, currentSpeaker=${_vm.state.currentSpeakerSeat}, subPhase=${_vm.state.currentSubPhase}');
  ...
  Future<void> setCurrentSpeaker(int? seatNumber) async {
    ...
    _vm.state = newState;
    AppLogger.d('setCurrentSpeaker completed, new speaker=${_vm.state.currentSpeakerSeat}');
  }
  ...
  void toggleRoleCard(int seatNumber) {
    AppLogger.d('toggleRoleCard: seat=$seatNumber, current showing=${_vm.state.showingRoleForSeat}');
  ...
  Future<void> onPlayerLongPress(int seatNumber, int actionType) async {
    ...
    switch (actionType) {
      ...
      default: AppLogger.d('  unknown actionType: $actionType');
    }
    await _vm.onCheckGameEnd();
  }
  ...
  Future<void> onNextTieCandidate() async {
    ...
    AppLogger.d('onNextTieCandidate completed, currentTieIndex=${_vm.state.currentTieIndex}');
  }
  ...
  Future<void> onCheckEliminationResult() async {
    ...
    AppLogger.d('onCheckEliminationResult completed, subPhase=${_vm.state.currentSubPhase}');
  }
