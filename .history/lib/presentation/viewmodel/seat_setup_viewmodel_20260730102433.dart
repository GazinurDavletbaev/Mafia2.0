...
class SeatSetupNotifier extends StateNotifier<SeatSetupState> {
  ...
  void updateFilteredList() {
    ...
    for (int i = 0; i < state.selectedPlayers.length; i++) {
      if (state.focusedIndex >= 0 && i == state.focusedIndex) continue;

      final selected = state.selectedPlayers[i];
      if (selected != null) {
  ...
  void onPlayerTap(int index, BuildContext context) {
    ...
    SeatSearchOverlay.show(
      ...
      onSelect: (member) {
        final newSelectedPlayers = List<Map<String, dynamic>?>.from(state.selectedPlayers);
...
// ===== PROVIDER =====
final seatSetupProvider = StateNotifierProvider<SeatSetupNotifier, SeatSetupState>((ref) {
  throw UnimplementedError('Use seatSetupProvider with params');
});

final seatSetupProviderFamily = StateNotifierProvider.family<SeatSetupNotifier, SeatSetupState, SeatSetupParams>(
