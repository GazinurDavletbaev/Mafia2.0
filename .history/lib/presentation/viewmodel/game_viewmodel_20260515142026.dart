...
class GameViewModel extends StateNotifier<GameState> {
  ...
  AppLogger.d('onPhaseBack: START, history length = ${_history.length}');
  
  if (_history.length <= 1) {
    AppLogger.d('onPhaseBack: history length <= 1, return');
    return;
  }
  
  final currentPhase = state.currentSubPhase;
  final currentSpeaker = state.currentSpeakerSeat;
  AppLogger.d('onPhaseBack: currentPhase = $currentPhase, currentSpeaker = $currentSpeaker');
  
  final lastSavedState = _history.last;
  final lastSavedPhase = lastSavedState.currentSubPhase;
  final lastSavedSpeaker = lastSavedState.currentSpeakerSeat;
  AppLogger.d('onPhaseBack: lastSavedPhase = $lastSavedPhase, lastSavedSpeaker = $lastSavedSpeaker');
  
  // Если текущий спикер null - просто удаляем последнее состояние и загружаем предыдущее
  if (currentSpeaker == null) {
    AppLogger.d('onPhaseBack: currentSpeaker is null, popping last state');
    _history.pop();
    final previousState = _history.last;
    state = previousState;
    AppLogger.d('onPhaseBack: new state phase = ${state.currentSubPhase}, speaker = ${state.currentSpeakerSeat}, history length = ${_history.length}');
    return;
  }
  ...
  if (currentPhase == lastSavedPhase && previousSpeaker == lastSavedSpeaker) {
    AppLogger.d('onPhaseBack: condition met, restoring last saved state without pop');
    state = lastSavedState;
    AppLogger.d('onPhaseBack: new state phase = ${state.currentSubPhase}, speaker = ${state.currentSpeakerSeat}');
    return;
  }
