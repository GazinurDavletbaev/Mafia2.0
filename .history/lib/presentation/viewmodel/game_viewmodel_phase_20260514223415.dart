...
AppLogger.d('phaseHistory before: ${_vm.state.phaseHistory.length}');
    final phaseHistory = _vm.state.phaseHistory;
    if (phaseHistory.isEmpty) return;

    // Запоминаем текущую и предыдущую фазу ДО удаления
    final currentPhaseToRemove = phaseHistory.last;
    final previousPhaseBefore = phaseHistory.length >= 2
        ? phaseHistory[phaseHistory.length - 2]
        : null;

    // Удаляем последнюю фазу
    ...
    final newDay = shouldDecrement
        ? _vm.state.currentDay - 1
        : _vm.state.currentDay;

    // Определяем currentPhase
    Phase newPhase;

    // Если удаляемая фаза была ночной, а предыдущая перед ней была дневной speeches
    if (currentPhaseToRemove == SubPhase.roleDistribution ||
        currentPhaseToRemove == SubPhase.contract ||
        currentPhaseToRemove == SubPhase.sheriffLook ||
        currentPhaseToRemove == SubPhase.freeSeating ||
        currentPhaseToRemove == SubPhase.mafiaShoot ||
        currentPhaseToRemove == SubPhase.donCheck ||
        currentPhaseToRemove == SubPhase.sheriffCheck) {
      // Возвращаемся из ночи в день
      newPhase = Phase.day;
    } else if (previousPhase == SubPhase.speeches &&
        previousPhaseBefore != null &&
        (previousPhaseBefore == SubPhase.mafiaShoot ||
            previousPhaseBefore == SubPhase.donCheck ||
            previousPhaseBefore == SubPhase.sheriffCheck)) {
      // Возвращаемся из дня в ночь (speeches → ночь)
      newPhase = Phase.night;
    } else if (previousPhase == SubPhase.mafiaShoot ||
