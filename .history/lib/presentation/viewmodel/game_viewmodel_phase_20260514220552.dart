...
class PhaseActions {
  ...
  final previousPhaseBefore = phaseHistory.length >= 2 ? phaseHistory[phaseHistory.length - 2] : null;

  // Удаляем последнюю фазу
  final newPhaseHistory = List<SubPhase>.from(phaseHistory)..removeLast();
  final previousPhase = newPhaseHistory.isNotEmpty ? newPhaseHistory.last : null;
  ...
  if (currentPhaseToRemove == SubPhase.mafiaShoot ||
      currentPhaseToRemove == SubPhase.donCheck ||
      currentPhaseToRemove == SubPhase.sheriffCheck) {
    // Возвращаемся из ночи в день
    newPhase = Phase.day;
  } else if (previousPhase == SubPhase.speeches && previousPhaseBefore != null &&
  ...
  final newState = _vm.state.copyWith(
    ...
    currentSpeakerSeat: _vm.state.speechHistory.isNotEmpty ? _vm.state.speechHistory.last : null,
  );
  ...
  Future<void> onPhaseForward() async {
    ...
    _vm.state = newState;
    AppLogger.d('after: currentSubPhase = ${newState.currentSubPhase}, day = $newDay');
  }
