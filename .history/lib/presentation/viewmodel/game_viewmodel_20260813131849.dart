...
class GameViewModel extends StateNotifier<GameState> {
  ...
  Future<void> onPhaseForward() async {
    ...
    switch (state.currentSubPhase) {
      ...
      final allFilled = state.players.every((p) => p.name.trim().isNotEmpty);
      
      if (!allFilled) {
        // ❌ НЕ ВСЕ ИГРОКИ ЗАПОЛНЕНЫ — ПОКАЗЫВАЕМ ОШИБКУ
        _showError('Посадите всех игроков за стол');
        return; // 🔥 ВЫХОДИМ, НЕ ПЕРЕХОДИМ ДАЛЬШЕ
      }
        _history.push(state);
