...
void main() {
  group('PlayerGrid отображение кандидатов', () {
    ...
    // Finder для центральной колонки - ищем Column внутри Row между двумя Expanded
    Finder getCenterColumnFinder(WidgetTester tester) {
      final row = find.byType(Row).first;
      final rowWidget = tester.widget<Row>(row);
      // Центральная колонка - вторая в children (индекс 1)
    ...
    testWidgets('voting — отображает nominatedSeats', (tester) async {
      ...
      final centerColumn = getCenterColumnFinder(tester);
