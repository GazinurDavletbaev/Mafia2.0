...
void main() {
  test(
      'БАГ: после eliminationVote с недостатком голосов -> ночь -> после стрельбы должна быть donCheck, а не voting',
      () async {
    ...
    expect(state.currentSubPhase, SubPhase.donCheck,
        reason:
            'После стрельбы должна быть donCheck, а не ${state.currentSubPhase}');
  });
}
