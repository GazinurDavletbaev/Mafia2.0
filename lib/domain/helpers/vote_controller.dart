...
class VoteController {
  ...
  static VoteResult determineResult(Map<int, int> votes, int aliveCount) {
    if (votes.isEmpty) return VoteResult.noCandidates();
    ...
    if (winners.length == 1) {
      ...
      // Проверяем, это перестрелка или голосование за подъём
      if (winners.length == 2) {
        return VoteResult.tieBreak(winners, maxVotes);
      } else {
        // 3+ кандидата с одинаковыми голосами → голосование за подъём
